export const defaults = {rows:4,words:7,width:540,x:50,y:50,step:1,wpm:120,style:'natural'};
...
export function layout(raw) {
  ...
  return {width:s.width,height,x:Math.round((576-s.width)*s.x/100),y:Math.round((288-height)*s.y/100)};
}
export function lines(text,raw) {
  const s=settings(raw),columns=Math.max(12,Math.floor((s.width-12)/14)),out=[];
...
export function frame(text,offset,raw) {
  const s=settings(raw),all=lines(text,s),last=Math.max(0,all.length-s.rows),start=Math.max(0,Math.min(last,offset));
