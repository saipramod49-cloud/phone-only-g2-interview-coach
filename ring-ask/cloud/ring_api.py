...
def handle(environ, start_response, credentials, conversation, model):
    ...
    if path == '/api/health' and method == 'GET':
        return reply('200 OK', {'configured': bool(credentials.get('api_key')), 'model':model, 'profile_loaded':ring_agent.bridge.EXPERIENCE.exists(), 'version':'0.5.0', 'answer_style':'spoken-flow-v1'})
