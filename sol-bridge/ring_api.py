...
def handle(environ, start_response, credentials, conversation, model):
    ...
    if path == '/api/health' and method == 'GET':
        return reply('200 OK', {'configured': bool(credentials.get('api_key')), 'model':model, 'profile_loaded':ring_agent.bridge.EXPERIENCE.exists(), 'version':'0.6.1', 'answer_style':'flow-spoken-keywords-v2'})
