...
export function mountPreparation(host:string,token:()=>string,preferred:()=>string=()=>'',changed:(id:string)=>void=()=>{}){
 ...
 <button id="load-prep">Load profiles</button><label id="profile-selector">Saved profile<select id="profiles"></select></label>
 ...
 const select=$<HTMLSelectElement>('#profiles');let profiles:Profile[]=[];let working=false;let loadedToken='';
 ...
 async function load(desired=select.value||preferred()){
 const key=token();const data=await api('/api/state');if(key!==token())return;
 profiles=data.profiles;select.replaceChildren();const selected=profiles.find(p=>p.id===desired)??profiles.find(p=>p.active)??profiles[0];
 for(const p of profiles){const option=document.createElement('option');option.value=p.id;option.textContent=p.name;option.selected=p.id===selected?.id;select.append(option);}
 loadedToken=key;list();changed(select.value);
 }
 ...
 select.onchange=()=>{list();changed(select.value);};
 $('#create-profile').onclick=()=>void run(async()=>{const name=$<HTMLInputElement>('#profile-name').value.trim();if(!name)throw new Error('Enter a profile name.');const form=new FormData();form.set('name',name);form.set('job_description',$<HTMLTextAreaElement>('#job-description').value);const created=await api('/api/profiles','POST',form);await load(created.id);});
 ...
 return {load:()=>run(()=>load()),ensure:async()=>{if(loadedToken!==token()||!select.value)await load();return select.value;},selected:()=>select.value};
}
