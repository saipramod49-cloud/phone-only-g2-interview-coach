...
it('honors a smaller character limit even for long technical terms',()=>{const s=new LiveState();s.charsPerLine=12;s.answer='BigQuery partitioning and idempotency';const lines=s.pages().join('\n').split('\n');expect(lines.every(line=>line.length<=12)).toBe(true);expect(normalizeDisplay({characters:1}).characters).toBe(12);});
