...
export function mountCoachInstructions(send:(text:string)=>boolean){
 ...
 <p>Latest conflicting suggestion wins. Suggestions stay while this page is open, including reconnects. Clear restores defaults. Language uses the answer-language selector. Highlighting uses UPPERCASE on the lens.</p></details>`;
 const $=<T extends HTMLElement=HTMLElement>(s:string)=>root.querySelector<T>(s)!;
 let active='';
 function sync(){$('#coach-status').textContent=send(active)?'Sending suggestions…':'Saved for this page. Start practice to apply.';}
 ...
 return {sync,ack:(enabled:boolean)=>{$('#coach-status').textContent=enabled?'Applied. Your next answer or Retry will use these suggestions.':'Suggestions cleared. Default answer style restored.';}};
}
